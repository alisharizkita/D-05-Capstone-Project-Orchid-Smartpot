  ...
  function MonitoringDashboard() {
    ...
    const [historicalData, setHistoricalData] = useState<{ labels: string[]; temperature: number[]; humidity: number[]; light_intensity: number[]; soil_moisture: number[]; }>({ labels: [], temperature: [], humidity: [], light_intensity: [], soil_moisture: [] });
    ...
    const fetchSensorData = async () => {
      try {
        ...
        if (json.status === "success" && json.data && json.data.data) {
          const allData = json.data.data; // array semua sensor
          const latest = allData[allData.length - 1]; // ambil data terbaru

          // 1) Set data realtime (monitoring)
          setMonitoringData({
            temperature: latest.temperature,
            humidity: latest.humidity,
            lightIntensity: latest.light_intensity,
            soilMoisture: latest.soil_moisture,
            ph: 0, // backend belum ada
            lastWatering: latest.formatted_time || '', // sementara gunakan formatted_time
            batteryLevel: 0, // mapping: water_level → batteryLevel
            connectionStatus: 'Connected'
          });

          // 2) Set data historis untuk chart
          const labels = allData.map(d => d.formatted_time);
          const temps = allData.map(d => d.temperature);
          const hums = allData.map(d => d.humidity);
          const lights = allData.map(d => d.light_intensity);
          const moistures = allData.map(d => d.soil_moisture);
