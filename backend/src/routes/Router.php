    ...
    public function dispatch(): array {
        ...
        if (count($parts) >= 1 && $parts[0] === 'users') {
            ...
            if ($this->method === 'OPTIONS') {
                return Response::success("CORS check success", [], 200); 
            }
        ...
        // ORCHID ROUTES
        // ============================================
        if (count($parts) >= 1 && end($parts) === 'orchid') {
            require_once __DIR__ . '/../controllers/OrchidController.php';
            $controller = new OrchidController($this->db);
            ...
            if ($this->method === 'GET') {
                ...
                return $controller->getAllOrchids($headers);
            }

            if ($this->method === 'POST') {
                $headers = getallheaders();
                $payload = json_decode(file_get_contents("php://input"), true) ?? [];
                return $controller->createOrchid($headers, $payload);
            }

            return Response::error("Invalid orchid endpoint", 404);
        }
